...
static void generate_sine_wave(int16_t *buffer, size_t samples_per_block, int sample_rate, int frequency, int channels)
{
...
static bool trigger_command(const struct device *dev_i2s,
...
void tone_thread(void *arg1, void *arg2, void *arg3) {
    ...
    while (stream_started) {
        ...
        generate_sine_wave(buffer, SAMPLES_PER_BLOCK, SAMPLE_FREQUENCY, SINE_FREQ, NUMBER_OF_CHANNELS);
        ...
        if (trigger_stream) {
            ...
            trigger_stream = false;  // Set false only after stream is triggered successfully
        }
...
static int cmd_start_tone(const struct shell *shell, size_t argc, char **argv) {
    ...
    k_thread_create(&tone_thread_data, tone_thread_stack,
                    ...
                    tone_thread, (void *)shell, NULL, NULL,
...
void main(void)
{
    ...
    printk("Shell initialized. Use 'start_tone' to start the tone and 'stop_tone' to stop it.\n");
}
